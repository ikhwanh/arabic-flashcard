...
- `#read` → **Yasin** tab — renders the first reading surah inline ([src/pages/reading.ts](src/pages/reading.ts) in embedded mode), falling back to the index grid when there is no reading data
...
The navigation page ([src/pages/navigation.ts](src/pages/navigation.ts)) has four tabs — **Flashcard** (deck grid, route ``), **Surah Breakdown** (route `#qs`), **Yasin** (route `#read`), and **Dzikir** (route `#dzikir`). The active tab is driven by the hash, not internal state, so each reader's back button returns to the right tab.
